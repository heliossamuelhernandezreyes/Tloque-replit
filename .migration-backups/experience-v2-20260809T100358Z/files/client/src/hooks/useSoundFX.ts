...
// Frecuencias base por género para el sonido de ciclo
const GENRE_FREQS: Record<string, number> = {
  general:     440,   // La — neutral
  melancolico: 330,   // Mi — grave, melancólico
  terror:      220,   // La grave — oscuro
  fantasia:    396,   // Sol — cálido, no invasivo
  misterio:    370,   // Fa# — tenso, ambiguo
  romance:     494,   // Si — cálido
}
...
function playTone(opts: {
  ...
  const userVol = getAudioSettings().volume
  osc.connect(gain)
  gain.connect(ctx.destination)
  // Aplicar volumen del usuario retroactivamente
  gain.gain.setValueAtTime(0, now)
  gain.gain.linearRampToValueAtTime(volume * userVol, now + attack)
  gain.gain.linearRampToValueAtTime(volume * userVol * sustain, now + attack + decay)
  gain.gain.linearRampToValueAtTime(0, now + attack + decay + release)
  osc.start(now)
  ...
  if (freq2) {
    ...
    gain2.gain.linearRampToValueAtTime(volume * 0.5 * userVol, now + attack)
    gain2.gain.linearRampToValueAtTime(volume * 0.15 * userVol, now + attack + decay)
...
function sound_book_complete() {
  ...
  notes.forEach(({ f, d }) => {
    ...
    g.gain.linearRampToValueAtTime(0.12 * volume, now + d + 0.02)
    ...
    g2.gain.linearRampToValueAtTime(0.04 * volume, now + d + 0.02)
...
function sound_streak_milestone() {
  ...
  notes.forEach((f, i) => {
    ...
    const g = ctx.createGain()
    g.gain.setValueAtTime(0, now + d)
    g.gain.linearRampToValueAtTime(0.11 * volume, now + d + 0.008)
...
function getAudioSettings(): { enabled: boolean; volume: number } {
  try {
    const raw = localStorage.getItem("novareads_settings")
    if (!raw) return { enabled: true, volume: 0.8 }
    const s = JSON.parse(raw)
    return {
      enabled: s.orbSounds !== false,
      volume:  typeof s.soundVolume === "number" ? s.soundVolume : 0.8,
    }
  } catch {
...
// Aplicar volumen global a un nodo de ganancia
function applyVolume(gainNode: GainNode, baseVolume: number) {
  const { volume } = getAudioSettings()
