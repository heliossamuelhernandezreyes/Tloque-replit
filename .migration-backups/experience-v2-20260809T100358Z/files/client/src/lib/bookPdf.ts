...
// ── Utilidades de carga (CDN, sin instalar paquetes) ─────────
function ensureScript(src: string, globalKey: string): Promise<void> {
  if ((window as any)[globalKey]) return Promise.resolve()
  return new Promise((resolve, reject) => {
    const s = document.createElement("script")
    s.src = src
    s.onload = () => resolve()
    s.onerror = () => reject(new Error(`No se pudo cargar ${globalKey}`))
...
function paint(
  ...
  if (copy) {
    ...
    // QR interno opcional: al escanearlo, la clave llega precargada
    // (esta página va DENTRO del libro; el QR de la portada no lleva clave)
...
export async function generateCoverKit(
  ...
  if (copy) {
    await ensureScript("https://cdnjs.cloudflare.com/ajax/libs/qrcode-generator/1.4.4/qrcode.min.js", "qrcode")
  }
