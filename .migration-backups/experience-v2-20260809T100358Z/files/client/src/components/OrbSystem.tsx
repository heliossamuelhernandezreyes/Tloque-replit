...
// ─────────────────────────────────────────────────────────
function CosmicEye({
  color, glow, pulse, searchMode,
}: {
  color: string; glow: string; pulse: boolean; searchMode: boolean
}) {
  ...
  return (
    ...
    >
      ...
      <motion.ellipse
        ...
        transition={{ duration: 7, repeat: Infinity, ease: "easeInOut", times: [0, 0.90, 0.945, 0.99, 1] }}
        style={{ transformOrigin: `${cx}px ${cy}px` }}
      />
    </svg>
  )
...
function RunicSeal({ color, glow, active, genre = "todos" }: { color: string; glow: string; active: boolean; genre?: string }) {
  ...
  return (
    <svg width="34" height="34" viewBox="0 0 34 34"
      ...
      <motion.g
        ...
        transition={{ duration: 12, repeat: active ? Infinity : 0, ease: "linear" }}
...
// ─────────────────────────────────────────────────────────
function PrismOrb({ color, glow, active }: { color: string; glow: string; active: boolean }) {
    ...
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}
      ...
      {v.map((vtx, i) => (
        <motion.circle key={i} cx={vtx.x} cy={vtx.y} r="1.3"
          ...
          transition={{ duration: 1.8 + i * 0.35, repeat: Infinity, ease: "easeInOut", delay: i * 0.2 }} />
      ))}
      <AnimatePresence>
        {active && (
...
export default function OrbSystem() {
  ...
  useEffect(() => {
    ...
    const allLocal = [...authored, ...saved].filter((b: any) => {
  ...
  useEffect(() => {
    ...
    if (pat && pat.length && "vibrate" in navigator) navigator.vibrate(pat)
  }, [activeGenre])
  ...
  function goToRandomBook() {
    const authored = JSON.parse(localStorage.getItem("novareads_authored") || "[]")
    const saved    = JSON.parse(localStorage.getItem("novareads_saved")    || "[]")
    // Intentar obtener libros del servidor desde localStorage caché
    const apiCache = (() => {
      try {
        // React Query guarda en su propio caché — intentar leer desde sessionStorage
        const cached = sessionStorage.getItem("novareads_api_books")
    ...
    play("navigate")
    if ("vibrate" in navigator) navigator.vibrate([18, 10, 18])
    flashOrb()
    closeSearch()
  ...
  return (
    <>
    <div className="fixed bottom-8 sm:bottom-12 left-0 right-0 flex justify-center z-[120] select-none pointer-events-none">
      <div className="relative flex items-center justify-center pointer-events-auto">
        ...
        >
          <RunicSeal color={cfg.color} glow={cfg.glow} active={activeGenre !== "todos"} genre={activeGenre} />
        </motion.button>
        ...
        <motion.button
          ...
          style={{ width: 34, height: 34 }}
        >
          <PrismOrb color={cfg.color} glow={cfg.glow} active={filterActive} />
        </motion.button>
        ...
        >
          <CosmicEye color={cfg.color} glow={cfg.glow} pulse={orbPulse} searchMode={searchMode} />
        </motion.button>
        ...
        {searchMode && createPortal(
          <AnimatePresence>
            <motion.div
              ...
              style={{
                position:       "fixed",
                bottom:         `${Math.round(window.innerHeight * 0.17)}px`,
                left:           0,
                right:          0,
                display:        "flex",
