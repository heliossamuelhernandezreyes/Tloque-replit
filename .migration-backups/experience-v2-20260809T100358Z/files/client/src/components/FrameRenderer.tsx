...
function physicalMaterial(m: any) {
  ...
  const base = preset
    ? MATERIALS[preset]
    : (() => {
        const c = typeof m?.baseColor === "string" ? m.baseColor : "#c9a84c"
        return {
  ...
  return {
    ...
    sparkleColor: typeof m?.lightSparkleColor === "string" ? m.lightSparkleColor : "#ffffff",
    keyColor: typeof m?.sceneKeyColor === "string" ? m.sceneKeyColor : "#fff4d6",
  }
}
...
export default function FrameRenderer({ preset, shape, className, nameText, children, asOverlay }: FrameRendererProps) {
  ...
  const th = {
    top:    Number(geo.thickness?.top)    || 38,
    right:  Number(geo.thickness?.right)  || 38,
    bottom: Number(geo.thickness?.bottom) || 38,
    left:   Number(geo.thickness?.left)   || 38,
  }
  const co = {
    tl: Number(geo.corners?.tl) || 44, tr: Number(geo.corners?.tr) || 44,
    br: Number(geo.corners?.br) || 44, bl: Number(geo.corners?.bl) || 44,
  }
  ...
  const sweepAngle = Number(m.lightSweepAngle)
  const sweepDeg   = Number.isFinite(sweepAngle) ? sweepAngle : -14

  // ── Destello del metal ──
  const shimmerOn  = m.shimmer !== false
  ...
  const glassTint = typeof glass.tint === "string" ? glass.tint : "#dceeff"
  const edgeGlow  = clamp(glass.edgeGlow, 0.24)
  const dispersion = clamp(glass.dispersion, 0.04)
  const dispColor = typeof glass.dispersionColor === "string" ? glass.dispersionColor : "#b8a7ff"

  // ── Halo (¡ahora también en las cartas!) ──
  const haloOn = halo.enabled !== false
  const haloColor = typeof halo.color === "string" ? halo.color : mat.light
  const haloInt = clamp(halo.intensity, 0.42)
  const haloShadow = haloOn
    ? `0 0 ${Math.round((Number(halo.spread) || 36) * 0.8)}px ${Math.round((Number(halo.softness) || 24) * 0.2)}px ${alpha(haloColor, haloInt * 0.75)}`
    : undefined

  // ── Canto (edgeStyle) ──
  const edgeAccent = typeof edge.accentColor === "string" ? edge.accentColor : mat.light
    ...
    <div
      ...
      <svg viewBox={`0 0 ${CW} ${CH}`} className="absolute inset-0 w-full h-full pointer-events-none"
        ...
        {Array.isArray(orn.shapes) && orn.shapes.slice(0, 40).map((sh: any, i: number) => {
          const d = stampPath(String(sh?.shape || "circle"), Number(sh?.x) || 0, Number(sh?.y) || 0, Number(sh?.size) || 30)
        ...
        {Array.isArray(orn.actions) && orn.actions.slice(0, 60).map((a: any, i: number) => {
          ...
          const d = "M" + pts.map((p: any) => `${Number(p?.x) || 0} ${Number(p?.y) || 0}`).join("L")
          return <path key={`a${i}`} d={d} fill="none" stroke={`url(#${id}-mat)`}
            strokeWidth={Number(a?.size) || 12} strokeLinecap="round" strokeLinejoin="round" />
        })}
      ...
      <style>{`
        ...
        .tqf-glass {
          ...
          will-change: transform;
                               ...
                               calc(var(--tq-ty, 0) * ${(CH * 0.04).toFixed(1)}px));
          will-change: transform;
        }
      `}</style>
    </div>
