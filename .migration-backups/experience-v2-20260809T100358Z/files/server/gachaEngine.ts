...
import {
  ...
  bookCards, books, userCards, unlockedBooks, walletLedger,
} from "@shared/schema"
import {
  RARITIES, RARITY_BY_KEY, TICKET, drawRarity, nextPity,
  type RarityKey,
} from "@shared/gacha"
...
export interface DrawPayload {
  ...
  paperGranted: number        // Papel de compensación si era repetida
...
export async function drawTicket(userId: number): Promise<DrawOutcome> {
  return await db.transaction(async (tx) => {
    ...
    const obras: any = await tx.execute(sql`
    ...
    const cartas: any = await tx.execute(sql`
    ...
    // ── 12. Entregar. Si es nueva: la carta Y el libro. Si repetida: Papel.
    let paperGranted = 0
    ...
    if (!isDuplicate) {
      ...
      paperGranted = RARITY_BY_KEY[draw.rarity].paperDup
      await tx.insert(walletLedger).values({
        userId, currency: "papel", delta: paperGranted,
        reason: "gacha_duplicate", refType: "gacha_card", refId: cardId,
      })
    }
