...
export function sanitizeCardFx(input: any): any {
  const layers = {
    back:  typeof input?.layers?.back  === "string" ? input.layers.back.slice(0, 400_000)  : "",
    mid:   typeof input?.layers?.mid   === "string" ? input.layers.mid.slice(0, 400_000)   : "",
    front: typeof input?.layers?.front === "string" ? input.layers.front.slice(0, 400_000) : "",
  }
  ...
  let frameId: number | null = Number(input?.frameId)
