...
export function floorFor(priceTinta: number): RarityKey {
  for (const p of PRICE_FLOOR) {
    if (priceTinta <= p.maxPrice) return p.minRarity
  }
  return "legendary"
}

const rIdx = (k: RarityKey) => RARITIES.findIndex(r => r.key === k)
...
export async function computeAllScores(): Promise<BookScore[]> {
  ...
  const raw: any = await db.execute(sql`
    ...
    lectura as (
      select rp.book_id::int as bid,
             count(*)                                             as lectores,
             count(*) filter (where rp.max_chapter + 1 >= c.total_caps) as terminaron
      from reading_progress rp
      join caps c on c.id = rp.book_id::int
      where rp.book_id ~ '^[0-9]+$'
      group by rp.book_id::int
    ),
    apoyo as (
