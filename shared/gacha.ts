...
export const RARITIES: Rarity[] = [
  { key: "common",    name: "Común",      prob: 0.5500, bonus:    1, priceMin:  30, priceMax:  45, sellable: true  },
  { key: "rare",      name: "Rara",       prob: 0.2500, bonus:    2, priceMin:  60, priceMax:  95, sellable: true  },
  { key: "very_rare", name: "Muy rara",   prob: 0.1200, bonus:    3, priceMin: 110, priceMax: 180, sellable: true  },
  { key: "unusual",   name: "Insólita",   prob: 0.0500, bonus:    7, priceMin: 200, priceMax: 330, sellable: true  },
  { key: "golden",    name: "Dorada",     prob: 0.0220, bonus:   19, priceMin: 400, priceMax: 650, sellable: true  },
  { key: "legendary", name: "Legendaria", prob: 0.0060, bonus:   66, priceMin: null, priceMax: null, sellable: false },
  { key: "mythic",    name: "Mítica",     prob: 0.0015, bonus:  317, priceMin: null, priceMax: null, sellable: false },
  { key: "absolute",  name: "Absoluta",   prob: 0.0005, bonus: 1586, priceMin: null, priceMax: null, sellable: false },
]
...
export const TICKET = {
  price:  10,   // 10 Tinta × $2 MXN = $20 MXN (aprox. 1 USD)
  direct:  3,   // 30% → al autor de la obra que salió
  pool:    4,   // 40% → al pozo (los bonos también terminan en autores)
  house:   3,   // 30% → a Tloque
} as const
...
export function drawRarity(input: DrawInput): DrawResult {
  ...
  // Si no existe carta Común, bajar ya no es posible. Elegimos entonces la
  // rareza disponible más baja que el pozo sí puede honrar; de otro modo el
  // 55% de las tiradas terminaría en reembolso aunque hubiera stock válido.
  if (!input.available[RARITIES[idx].key]) {
    const replacement = RARITIES.findIndex(r =>
      input.available[r.key] === true && input.poolBalance >= r.bonus)
    if (replacement >= 0) {
      downgraded.push(RARITIES[idx].key)
      idx = replacement
      reason = "no_stock"
    }
  }
