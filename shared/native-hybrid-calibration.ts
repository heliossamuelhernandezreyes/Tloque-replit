...
export type CalibratedHybridSource = NativeHybridSource & { calibrationTuning?: HybridCalibrationTuning }
...
export function worstHybridCalibrationFailure(report: HybridAbValidationReport) {
  ...
  for (const cell of report.cellResults ?? []) for (const metric of cell.metrics) {
    const deficit = metricDeficit(metric)
    if (deficit > 0 && (!worst || deficit > worst.deficit)) worst = { cell, metric, deficit }
  }
  return worst
}
function step(deficit: number, min: number, max: number) { return clamp(min + deficit * (max - min) * 0.55, min, max) }
function familySpecificTuning(source: NativeHybridSource, metric: HybridAbMetric, deficit: number): HybridCalibrationTuning {
  ...
  if (source.physicalLayer === "bowed-string-resonator") {
    if (metric.id === "spectral-deviation") { tuning.textureScale = 1 - step(deficit, 0.035, 0.10); tuning.dampingScale = 1 - step(deficit, 0.025, 0.075) }
    else if (metric.id === "transient-preservation") tuning.textureScale = 1 - step(deficit, 0.025, 0.075)
    else if (metric.id === "dynamic-response") tuning.feedbackScale = 1 - step(deficit, 0.008, 0.025)
    else if (metric.id === "sustain-continuity") { tuning.feedbackScale = 1 + step(deficit, 0.008, 0.025); tuning.bodyScale = 1 + step(deficit, 0.025, 0.075) }
    else if (metric.id === "tail-naturalness") { tuning.feedbackScale = 1 + step(deficit, 0.008, 0.022); tuning.decayScale = 1 + step(deficit, 0.025, 0.075) }
  } else if (source.physicalLayer === "air-column-resonator") {
    if (metric.id === "spectral-deviation") { tuning.dampingScale = 1 - step(deficit, 0.025, 0.08); tuning.textureScale = 1 - step(deficit, 0.025, 0.075) }
    else if (metric.id === "transient-preservation") tuning.textureScale = 1 - step(deficit, 0.025, 0.07)
    else if (metric.id === "dynamic-response") tuning.feedbackScale = 1 - step(deficit, 0.008, 0.022)
    else if (metric.id === "sustain-continuity") { tuning.feedbackScale = 1 + step(deficit, 0.008, 0.024); tuning.bodyScale = 1 + step(deficit, 0.02, 0.06) }
    else if (metric.id === "tail-naturalness") { tuning.feedbackScale = 1 + step(deficit, 0.008, 0.022); tuning.decayScale = 1 + step(deficit, 0.02, 0.065) }
  } else {
    if (metric.id === "spectral-deviation") { tuning.bodyScale = 1 - step(deficit, 0.025, 0.075); tuning.dampingScale = 1 - step(deficit, 0.02, 0.06) }
    else if (metric.id === "transient-preservation") tuning.bodyScale = 1 - step(deficit, 0.02, 0.06)
    else if (metric.id === "dynamic-response") tuning.bodyScale = 1 - step(deficit, 0.015, 0.05)
    else if (metric.id === "sustain-continuity") { tuning.bodyScale = 1 + step(deficit, 0.025, 0.075); tuning.decayScale = 1 + step(deficit, 0.02, 0.06) }
    else if (metric.id === "tail-naturalness") { tuning.decayScale = 1 + step(deficit, 0.03, 0.09); tuning.bodyScale = 1 + step(deficit, 0.02, 0.055) }
  }
...
export function hybridCalibrationScore(report: HybridAbValidationReport) {
  ...
  let worstMargin = Infinity, passingCells = 0
