...
export function createSceneWeather(world: Scene) {
  ...
  for (let i = 0; i < count; i++) { vertices.push(random(i * 4) * 4 - 2, random(i * 4 + 1) * 5 - 2.5, random(i * 4 + 2) * 2 + .8); seeds.push(random(i * 4 + 3)) }
  ...
  const material = new ShaderMaterial({ transparent: true, depthWrite: false, depthTest: true, uniforms: { uTime: { value: 0 }, uKind: { value: 0 }, uIntensity: { value: .5 }, uColor: { value: new Color("#badbff") } }, vertexShader: `
    attribute float seed; uniform float uTime; uniform float uKind; uniform float uIntensity; varying float vFade; varying float vSeed;
    void main(){ vec3 p=position; float t=uTime; float life=fract(seed+t*.22);
      ...
      else if(uKind<2.5){p.x=position.x*.65+sin(t*2.+seed*18.)*.12;p.y=-2.35+life*2.4;p.z=1.6+seed;}
      ...
      float s=uKind>4.5&&uKind<5.5?44.:uKind>1.5&&uKind<2.5?72.+seed*24.:uKind<1.5?24.:5.;
      gl_PointSize=clamp(s*(3.8/-mv.z),1.,96.);vFade*=uIntensity;vSeed=seed;
    }`, fragmentShader: `
    uniform vec3 uColor; uniform float uKind; uniform float uTime; varying float vFade; varying float vSeed;
    void main(){vec2 p=gl_PointCoord-.5;if(uKind<1.5)p.x*=8.;
      ...
      if(uKind>1.5&&uKind<2.5){
        float h=1.-gl_PointCoord.y;float sway=.10*sin(h*5.+uTime*3.+vSeed*30.)*h;
        float width=.32*pow(max(0.,1.-h),.65);
        a=(1.-smoothstep(width*.45,width+.035,abs(p.x-sway)))*smoothstep(0.,.16,h)*(1.-smoothstep(.82,1.,h));
        c=mix(vec3(1.,.9,.5),uColor,smoothstep(.1,.8,h));
      }
