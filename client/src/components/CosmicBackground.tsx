...
export default function CosmicBackground() {
  ...
  useEffect(() => {
    ...
    function createParticle(randomY = false): Particle {
      return {
        ...
        y:       randomY ? Math.random() * canvasEl.height : -10,
    ...
    // En reducción de movimiento dejamos un fondo estable. No mantenemos un
    // requestAnimationFrame vacío consumiendo batería.
    ...
    function tick() {
      ...
      for (const p of state.particles) {
        ...
        context.arc(p.x, p.y, drawSize, 0, Math.PI * 2)
        context.fillStyle = `rgba(${Math.round(r)},${Math.round(g)},${Math.round(b)},${drawOp})`
        context.fill()
      }
    ...
    function start() {
      if (!state.animId && document.visibilityState !== "hidden") tick()
    }
    function stop() {
  ...
  }, [settings.reduceMotion]) // El color se maneja en stateRef; el presupuesto de movimiento sí reinicia el loop.
