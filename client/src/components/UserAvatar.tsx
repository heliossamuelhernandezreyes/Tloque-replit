...
export default function UserAvatar({
  ...
  const galleryId = /^gallery:(\d+)$/.exec(frame || "")?.[1]
  const galleryFrame = byId(galleryId ? Number(galleryId) : null)
  ...
  if (galleryFrame) {
    return (
      <div className={className} style={{ width: size, height: size }}>
        <FrameRenderer preset={galleryFrame.pkg} shape="profile" className="w-full h-full">
          {inner}
        </FrameRenderer>
      </div>
    )
