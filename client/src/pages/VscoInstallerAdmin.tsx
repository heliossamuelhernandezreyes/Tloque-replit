...
export default function VscoInstallerAdmin() {
  ...
  const packs = CURATED_SAMPLE_PACKS.filter(pack => pack.libraryName === "VSCO 2 Community Edition")
  ...
  return (
    <main className="min-h-screen bg-zinc-950 pb-12 text-zinc-200">
      <header className="sticky top-0 z-20 flex items-center gap-3 border-b border-white/10 bg-zinc-950/95 px-4 py-3">
        ...
        <div><h1 className="font-semibold">VSCO Strings</h1><p className="text-[10px] text-zinc-500">Familia acústica nativa de Tloque</p></div>
      </header>
      ...
      <section className="mx-auto max-w-3xl space-y-4 p-4">
        <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/[0.035] p-4 text-xs leading-5 text-zinc-400">
          ...
          <p className="mt-1">Cada instrumento se descarga sólo cuando lo necesitas. Tloque usa el commit fijado de VSCO 2 CE, interpreta el SFZ como datos inertes, valida RIFF/WAVE, calcula SHA-256 y deduplica las muestras en App Storage.</p>
          <p className="mt-2 text-zinc-500">La identidad refleja la grabación original: violín y contrabajo son solistas; viola y cello son secciones.</p>
        </div>
              ...
              <article key={pack.id} className="rounded-2xl border border-amber-300/20 bg-amber-300/[0.04] p-4 space-y-4">
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-[11px] text-zinc-500">{pack.instrumentId} · {pack.sfzPath}</p>
                  </div>
                </div>
