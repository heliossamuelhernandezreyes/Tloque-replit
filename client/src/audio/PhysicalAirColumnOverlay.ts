...
/** Dispatcher for non-bowed hybrid layers. */
export function scheduleAirColumnOverlay(context: BaseAudioContext, source: NativeHybridSource, options: AirColumnOverlayOptions) {
  ...
  const profile = profileFor(source), hz = midiHz(midi), start = startAt + event.timeSeconds
  const duration = Math.max(0.04, event.durationSeconds), release = event.articulation === "legato" ? 0.2 : 0.12, stop = start + duration + release
  const pressure = clamp01(event.velocity * 0.64 + track.expression * 0.36), brightness = clamp01(track.brightness), vibrato = clamp01(track.vibrato)

  const excitation = context.createGain(), pressureGain = context.createGain(); excitation.gain.value = 1; pressureGain.gain.value = 0.5 + pressure * 0.5; excitation.connect(pressureGain)
  const fundamental = context.createOscillator(), fg = context.createGain(); fundamental.type = source.instrumentId.startsWith("brass.") ? "sawtooth" : "triangle"; fundamental.frequency.value = hz; fg.gain.value = 0.14 + pressure * 0.1; fundamental.connect(fg); fg.connect(excitation)
  const h2 = context.createOscillator(), h2g = context.createGain(); h2.type = "sine"; h2.frequency.value = hz * 2; h2g.gain.value = profile.h2 * (0.7 + pressure * 0.3); h2.connect(h2g); h2g.connect(excitation)
  const h3 = context.createOscillator(), h3g = context.createGain(); h3.type = "sine"; h3.frequency.value = hz * 3; h3g.gain.value = profile.h3 * (0.72 + brightness * 0.28); h3.connect(h3g); h3g.connect(excitation)
  const breath = context.createBufferSource(), breathBand = context.createBiquadFilter(), breathGain = context.createGain(); breath.buffer = noiseBuffer(context); breath.loop = true; breathBand.type = "bandpass"; breathBand.frequency.value = profile.damping * (0.55 + brightness * 0.5); breathBand.Q.value = 0.65; breathGain.gain.value = profile.noise * (0.4 + pressure * 0.6); breath.connect(breathBand); breathBand.connect(breathGain); breathGain.connect(excitation)

  const delay = context.createDelay(0.09), damping = context.createBiquadFilter(), feedback = context.createGain(); const baseDelay = Math.min(0.08, Math.max(1 / 18_000, 1 / hz)); delay.delayTime.value = baseDelay; damping.type = "lowpass"; damping.frequency.value = profile.damping * (0.72 + brightness * 0.5); damping.Q.value = 0.25; feedback.gain.value = profile.feedback * (0.93 + pressure * 0.05); excitation.connect(delay); delay.connect(damping); damping.connect(feedback); feedback.connect(delay)
  const bus = context.createGain(), tap = context.createGain(); bus.gain.value = 1; tap.gain.value = 0.24; damping.connect(tap); tap.connect(bus)
  for (const frequency of profile.formants) { const filter = context.createBiquadFilter(), gain = context.createGain(); filter.type = "bandpass"; filter.frequency.value = frequency; filter.Q.value = 2.1; gain.gain.value = 0.095; damping.connect(filter); filter.connect(gain); gain.connect(bus) }
  ...
  for (const control of controls) {
    ...
    if (control.expression !== null) { const p = clamp01(event.velocity * 0.64 + control.expression * 0.36); scheduleParam(pressureGain.gain, at, 0.5 + p * 0.5, control.rampSeconds); scheduleParam(feedback.gain, at, profile.feedback * (0.93 + p * 0.05), control.rampSeconds); scheduleParam(breathGain.gain, at, profile.noise * (0.4 + p * 0.6), control.rampSeconds) }
    if (control.brightness !== null) { const b = clamp01(control.brightness); scheduleParam(damping.frequency, at, profile.damping * (0.72 + b * 0.5), control.rampSeconds, true); scheduleParam(breathBand.frequency, at, profile.damping * (0.55 + b * 0.5), control.rampSeconds, true) }
  ...
  const attack = legatoFromPrevious ? 0.009 : Math.max(0.016, Math.min(0.065, track.attack * 0.35)), peak = source.wet * (0.68 + pressure * 0.32)
  output.gain.setValueAtTime(0.0001, start); output.gain.exponentialRampToValueAtTime(Math.max(0.001, peak), start + attack); output.gain.setValueAtTime(Math.max(0.001, peak * 0.9), Math.max(start + attack, start + duration - 0.012)); output.gain.exponentialRampToValueAtTime(0.0001, stop)
