...
function wetGainFor(source: NativeHybridSource, pressure: number, embouchure: number) { return Math.max(0.001, source.wet * (0.65 + pressure * 0.3 + embouchure * 0.05)) }
...
export function scheduleAirColumnOverlay(context: BaseAudioContext, source: NativeHybridSource, options: AirColumnOverlayOptions) {
  ...
  const h2 = context.createOscillator(), h2g = context.createGain(); h2.type = "sine"; h2.frequency.value = hz * 2; h2g.gain.value = profile.h2 * (0.68 + pressure * 0.32) * embouchureTone(embouchure); h2.connect(h2g); h2g.connect(excitation)
  ...
  const delay = context.createDelay(0.09), damping = context.createBiquadFilter(), feedback = context.createGain(); const baseDelay = Math.min(0.08, Math.max(1 / 18_000, 1 / hz)); delay.delayTime.value = baseDelay; damping.type = "lowpass"; damping.frequency.value = profile.damping * (0.64 + brightness * 0.35 + embouchure * 0.28); damping.Q.value = 0.25; feedback.gain.value = profile.feedback * (0.91 + pressure * 0.055) * embouchureFeedback(embouchure); excitation.connect(delay); delay.connect(damping); damping.connect(feedback); feedback.connect(delay)
  ...
  for (const frequency of profile.formants) {
    const filter = context.createBiquadFilter(), gain = context.createGain(); filter.type = "bandpass"; filter.frequency.value = frequency * (0.94 + embouchure * 0.12); filter.Q.value = 2.1; gain.gain.value = 0.095; damping.connect(filter); filter.connect(gain); gain.connect(bus); formantFilters.push(filter)
  }
  ...
  output.gain.setValueAtTime(0.0001, start); output.gain.exponentialRampToValueAtTime(wetGainFor(source, pressure, embouchure), start + attack)
  ...
  for (const control of controls) {
    ...
    scheduleParam(h2g.gain, at, profile.h2 * (0.68 + p * 0.32) * embouchureTone(e), control.rampSeconds)
    scheduleParam(h3g.gain, at, profile.h3 * (0.7 + b * 0.24 + e * 0.12), control.rampSeconds)
    scheduleParam(feedback.gain, at, profile.feedback * (0.91 + p * 0.055) * embouchureFeedback(e), control.rampSeconds)
    scheduleParam(breathGain.gain, at, profile.noise * (0.35 + p * 0.65) * (1.1 - e * 0.16), control.rampSeconds)
    scheduleParam(damping.frequency, at, profile.damping * (0.64 + b * 0.35 + e * 0.28), control.rampSeconds, true)
    scheduleParam(breathBand.frequency, at, profile.damping * (0.48 + b * 0.35 + e * 0.28), control.rampSeconds, true)
    ...
    scheduleParam(output.gain, at, wetGainFor(source, p, e), control.rampSeconds, true)
  ...
  const endState = physicalPerformanceStateAt(track, controls, event.timeSeconds + duration), endPressure = pressureFor(event.velocity, endState.pressure), finalPeak = wetGainFor(source, endPressure, endState.embouchure)
