...
export function scheduleAirColumnOverlay(context: BaseAudioContext, source: NativeHybridSource, options: AirColumnOverlayOptions) {
  ...
  const pressure = clamp01(event.velocity * 0.44 + state0.pressure * 0.56), embouchure = state0.embouchure, brightness = clamp01(track.brightness), vibrato = clamp01(track.vibrato)
  const embouchureTone = 0.78 + embouchure * 0.42
  const embouchureFeedback = 0.94 + (1 - Math.abs(embouchure - 0.5) * 2) * 0.06
  ...
  const h2 = context.createOscillator(), h2g = context.createGain(); h2.type = "sine"; h2.frequency.value = hz * 2; h2g.gain.value = profile.h2 * (0.68 + pressure * 0.32) * embouchureTone; h2.connect(h2g); h2g.connect(excitation)
  const h3 = context.createOscillator(), h3g = context.createGain(); h3.type = "sine"; h3.frequency.value = hz * 3; h3g.gain.value = profile.h3 * (0.7 + brightness * 0.24 + embouchure * 0.12); h3.connect(h3g); h3g.connect(excitation)
  const breath = context.createBufferSource(), breathBand = context.createBiquadFilter(), breathGain = context.createGain(); breath.buffer = noiseBuffer(context); breath.loop = true; breathBand.type = "bandpass"; breathBand.frequency.value = profile.damping * (0.48 + brightness * 0.35 + embouchure * 0.28); breathBand.Q.value = 0.65; breathGain.gain.value = profile.noise * (0.35 + pressure * 0.65) * (1.1 - embouchure * 0.16); breath.connect(breathBand); breathBand.connect(breathGain); breathGain.connect(excitation)

  const delay = context.createDelay(0.09), damping = context.createBiquadFilter(), feedback = context.createGain(); const baseDelay = Math.min(0.08, Math.max(1 / 18_000, 1 / hz)); delay.delayTime.value = baseDelay; damping.type = "lowpass"; damping.frequency.value = profile.damping * (0.64 + brightness * 0.35 + embouchure * 0.28); damping.Q.value = 0.25; feedback.gain.value = profile.feedback * (0.91 + pressure * 0.055) * embouchureFeedback; excitation.connect(delay); delay.connect(damping); damping.connect(feedback); feedback.connect(delay)
  ...
  for (const frequency of profile.formants) { const filter = context.createBiquadFilter(), gain = context.createGain(); filter.type = "bandpass"; filter.frequency.value = frequency * (0.94 + embouchure * 0.12); filter.Q.value = 2.1; gain.gain.value = 0.095; damping.connect(filter); filter.connect(gain); gain.connect(bus) }
  ...
  for (const control of controls) {
    ...
    const p = clamp01(event.velocity * 0.44 + state.pressure * 0.56), e = state.embouchure
    scheduleParam(pressureGain.gain, at, 0.46 + p * 0.54, control.rampSeconds)
    scheduleParam(feedback.gain, at, profile.feedback * (0.91 + p * 0.055) * (0.94 + (1 - Math.abs(e - 0.5) * 2) * 0.06), control.rampSeconds)
    scheduleParam(breathGain.gain, at, profile.noise * (0.35 + p * 0.65) * (1.1 - e * 0.16), control.rampSeconds)
    scheduleParam(damping.frequency, at, profile.damping * (0.64 + (control.brightness ?? brightness) * 0.35 + e * 0.28), control.rampSeconds, true)
    scheduleParam(breathBand.frequency, at, profile.damping * (0.48 + (control.brightness ?? brightness) * 0.35 + e * 0.28), control.rampSeconds, true)
  ...
  const attack = legatoFromPrevious ? 0.009 : Math.max(0.016, Math.min(0.065, track.attack * 0.35)), peak = source.wet * (0.65 + pressure * 0.3 + embouchure * 0.05)
