...
This historical revision changed the air-column and sympathetic families. Bowed strings remained `bowed-string-overlay-v1` at that point; Orchestral Hybrid V4 later superseded it with a continuous waveguide. Intelligent Performer V5 used `bowed-string-overlay-v3-intelligent-gesture` and `air-column-overlay-v1.2-intelligent-gesture`. Orchestra Conductor V7 now uses `bowed-string-overlay-v5-acoustic-continuity` and `air-column-overlay-v1.4-acoustic-continuity`, intentionally invalidating all earlier A/B evidence for promotion.
