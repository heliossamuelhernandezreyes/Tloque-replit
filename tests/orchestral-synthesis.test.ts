...
test("realtime y WAV consumen las mismas voces, interpretación y envolventes", () => {
  for (const file of ["NativeSampleScoreEngine.ts", "NativeSampleScoreExporter.ts"]) {
    ...
    assert.match(code, /nativeSampleVoiceEnvelope\(voice\)/)
    assert.match(code, /fallbackTrackIds.has\(event.trackId\)/)
  }
  const sharedEnvelope = readFileSync("client/src/audio/NativeSampleScorePlan.ts", "utf8")
  assert.match(sharedEnvelope, /expression: voice.expression/)
  assert.match(sharedEnvelope, /dynamics: voice.dynamics/)
  assert.match(sharedEnvelope, /layerGainCurve: voice.layerGainCurve/)
  assert.match(readFileSync("client/src/audio/NativeSampleScoreEngine.ts", "utf8"), /scoreMonitorVolume\(this.master/)
  assert.equal(ORCHESTRAL_SYNTH_VERSION, "tloque-orchestral-synth-v5-acoustic-continuity")
})
