...
import { frameSceneSchema, packageFrameScene } from "../shared/frame-scene"
...
export function validateFrame(input: any): { ok: true; frame: any } | { ok: false; error: string } {
  ...
  if (safePkg.scene !== undefined || safePkg.renderer === "tloque-scene-v2" || safePkg.schemaVersion === "2.0.0") {
    const scene = frameSceneSchema.safeParse(safePkg.scene)
    if (!scene.success) return { ok: false, error: "La escena 3D contiene parámetros o una secuencia no válidos" }
    const canonical = packageFrameScene(scene.data, name, target)
    return { ok: true, frame: { name, priceTinta, target, schemaVersion: canonical.schemaVersion, fingerprint: "", pkg: canonical } }
  }
