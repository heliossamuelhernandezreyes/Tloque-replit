...
async function cardEditor(page, create = true) {
  ...
  if (create) {
    ...
    await page.getByLabel("Fondo de la tarjeta", { exact: true }).setInputFiles({ name: "back.png", mimeType: "image/png", buffer: Buffer.from(assets.back.split(",")[1], "base64") })
  ...
  await page.getByRole("button", { name: "Diseñar tarjeta, marco y efectos", exact: true }).click()
...
try {
  ...
  await slider(page,"Ancho del marco",.14)
  await page.getByRole("button",{name:"Mica",exact:true}).click()
  await page.getByLabel("Acabado de mica",{exact:true}).selectOption("holographic")
  ...
  assert.equal(saved.pkg.scene.portalCard.frame.width,.14)
  assert.equal(saved.pkg.scene.portalCard.mica.finish,"holographic")
  await proof(page,"portal-frame-front")
  await page.locator(".tq-turn-controls").getByRole("button",{name:"Reverso",exact:true}).click()
  await proof(page,"portal-frame-back")
  await page.getByText("Plantillas anteriores · ornamentos 3D",{exact:true}).click()
  ...
  await page.getByRole("button", { name: "Atlas astral", exact: false }).first().click()
  await page.locator('.tq-studio-header input[type="file"]').setInputFiles(filePath)
  ...
  await mobile.page.getByLabel("Pista", { exact: true }).selectOption("orbit")
  ...
  await director.page.getByText("Compatibilidad con escenas anteriores",{exact:true}).click()
  await director.page.getByRole("button",{name:"Abrir modo anterior",exact:true}).click()
  await director.page.getByRole("button",{name:"Composición",exact:true}).first().click()
  ...
  assert.equal(cardRecipe.version, 2)
  assert.equal(cardRecipe.images.length, 3)
  assert.equal(cardRecipe.images[1], "")
  assert.ok(cardRecipe.images[0].startsWith("data:image/"))
  assert.ok(cardRecipe.images[2].startsWith("data:image/"))
  await director.page.getByRole("button", { name: "Viaje astral", exact: false }).click()
  await director.page.locator('.tq-card-director .tq-studio-header input[type="file"]').setInputFiles(cardRecipePath)
  await director.page.getByRole("status").filter({ hasText: "Tarjeta importada con sus imágenes" }).waitFor()
  ...
  assert.deepEqual([savedCard.fx.layers.back, savedCard.fx.layers.mid, savedCard.fx.layers.front], cardRecipe.images)
  ...
  console.log("QA: portal 360, mica, three weather zones, back, import and persistence")
  const portal=await setup({strictAssets:true})
  await cardEditor(portal.page);await rendered(portal.page)
  const cutout=await portal.page.evaluate(()=>{
    const c=document.createElement("canvas");c.width=700;c.height=980;const g=c.getContext("2d")
    g.fillStyle="#759a9f";g.beginPath();g.moveTo(0,680);g.lineTo(180,380);g.lineTo(370,690);g.lineTo(560,270);g.lineTo(700,570);g.lineTo(700,980);g.lineTo(0,980);g.fill();return c.toDataURL("image/png")
  })
  await portal.page.getByLabel("Importar capa media",{exact:true}).setInputFiles({name:"mountains.png",mimeType:"image/png",buffer:Buffer.from(cutout.split(",")[1],"base64")})
  await portal.page.getByRole("dialog").getByText(/Transparencia conservada/).waitFor()
  await portal.page.getByRole("button",{name:"Quitar capa media",exact:true}).click()
  assert.match(await portal.page.locator(".tq-card-layer-count").innerText(), /2\s*\/\s*3/)
  await portal.page.getByRole("button",{name:"Deshacer dirección",exact:true}).click()
  assert.match(await portal.page.locator(".tq-card-layer-count").innerText(), /3\s*\/\s*3/)
  await portal.page.getByRole("button",{name:"Marco",exact:true}).click()
  await portal.page.getByRole("button",{name:"Oro",exact:true}).click()
  await slider(portal.page,"Ancho del marco",.12)
  await slider(portal.page,"Grosor de la tarjeta",.22)
  await slider(portal.page,"Iluminación del borde",.35)
  await portal.page.getByRole("button",{name:"Mica",exact:true}).click()
  await portal.page.getByLabel("Acabado de mica",{exact:true}).selectOption("holographic")
  await portal.page.getByLabel("Superficie de mica",{exact:true}).selectOption("drops")
  await portal.page.getByRole("button",{name:"Clima",exact:true}).click()
  await portal.page.getByLabel("Clima en las tres zonas",{exact:true}).selectOption("rain")
  await portal.page.getByRole("button",{name:"Entre la capa media y el primer plano",exact:true}).click()
  await portal.page.getByLabel("Efecto de la zona",{exact:true}).selectOption("snow")
  await slider(portal.page,"Tamaño de partículas",1.65)
  await portal.page.getByRole("button",{name:"Delante del primer plano",exact:true}).click()
  await portal.page.getByLabel("Efecto de la zona",{exact:true}).selectOption("magic")
  await portal.page.locator(".tq-portal-tabs").getByRole("button",{name:"Reverso",exact:true}).click()
  await portal.page.getByLabel("Título del reverso",{exact:true}).fill("LA PUERTA DE NIEVE")
  await portal.page.getByLabel("Inscripción del reverso",{exact:true}).fill("Una historia al otro lado · 雪 ❄")
  await slider(portal.page,"Tiempo de inspección",2.7)
  await proof(portal.page,"portal-layered-front")
  const turntable=portal.page.getByRole("group",{name:/Girar tarjeta 360/})
  await turntable.focus();await portal.page.keyboard.press("Shift+ArrowRight")
  await proof(portal.page,"portal-parallax-45")
  await portal.page.keyboard.press("Shift+ArrowRight")
  await proof(portal.page,"portal-edge-90")
  await portal.page.keyboard.press("Shift+ArrowRight")
  await portal.page.keyboard.press("Shift+ArrowRight")
  assert.equal(await turntable.getAttribute("data-card-face"),"back")
  await proof(portal.page,"portal-engraved-back")
  for(let i=0;i<4;i++)await portal.page.keyboard.press("Shift+ArrowRight")
  assert.equal(await turntable.getAttribute("data-card-face"),"front")
  await portal.page.locator(".tq-turn-controls").getByRole("button",{name:"Frente",exact:true}).click()
  await portal.page.getByRole("button",{name:"Imágenes",exact:true}).click()
  await portal.page.getByLabel("Capa activa",{exact:true}).selectOption("back")
  for(const [name,value] of [["Posición X de capa",.6],["Posición Y de capa",.6],["Rotación de capa",45],["Escala de capa",.5]])await slider(portal.page,name,value)
  assert.equal(await portal.page.getByLabel("Opacidad de capa",{exact:true}).isDisabled(),true)
  await proof(portal.page,"portal-background-extreme")
  await portal.page.getByRole("button",{name:"Restablecer encuadre",exact:true}).click()
  const portalDownload=portal.page.waitForEvent("download")
  await portal.page.getByRole("button",{name:"Exportar dirección",exact:true}).click()
  const portalRecipePath=await(await portalDownload).path(),portalRecipe=JSON.parse(await readFile(portalRecipePath,"utf8"))
  assert.equal(portalRecipe.scene.portalCard.frame.thickness,.22)
  assert.equal(portalRecipe.images.filter(Boolean).length,3)
  assert.deepEqual(portalRecipe.scene.portalCard.weather.map(z=>z.type),["rain","snow","magic"])
  assert.equal(portalRecipe.scene.portalCard.back.title,"LA PUERTA DE NIEVE")
  await portal.page.getByRole("button",{name:"Marco",exact:true}).click()
  await slider(portal.page,"Grosor de la tarjeta",.09)
  await portal.page.locator('.tq-studio-header input[type="file"]').setInputFiles(portalRecipePath)
  await portal.page.getByRole("status").filter({hasText:"Tarjeta importada con sus imágenes"}).waitFor()
  await portal.page.getByRole("button",{name:"Usar esta dirección",exact:true}).click()
  await portal.page.getByRole("button",{name:"Crear tarjeta",exact:true}).click()
  await portal.page.getByRole("button",{name:"Editar Portal de prueba",exact:true}).waitFor()
  assert.deepEqual(savedCard.fx.scene,portalRecipe.scene)
  assert.deepEqual([savedCard.fx.layers.back,savedCard.fx.layers.mid,savedCard.fx.layers.front],portalRecipe.images)
  assert.ok(savedCard.fx.layers.mid.startsWith("data:image/"))
  await portal.page.reload()
  await cardEditor(portal.page,false);await rendered(portal.page)
  await portal.page.locator(".tq-turn-controls").getByRole("button",{name:"Reverso",exact:true}).click()
  await proof(portal.page,"portal-reloaded-back")
  ...
  for(let i=1;i<=8;i++)await touch.send("Input.dispatchTouchEvent",{type:"touchMove",touchPoints:[{...start,x:start.x+i*30}]})
  await touch.send("Input.dispatchTouchEvent",{type:"touchEnd",touchPoints:[]})
  assert.equal(await mobileTurn.getAttribute("data-card-face"),"back")
  await proof(portalMobile.page,"portal-mobile-back")
  ...
  await portalEssential.page.locator(".tq-turn-controls").getByRole("button",{name:"Reverso",exact:true}).click()
  await portalEssential.page.locator(".tq-portal-back-poster").getByText("LA PUERTA DE NIEVE",{exact:true}).waitFor()
  assert.equal(await portalEssential.page.locator("canvas").count(),0)
  await portalEssential.context.close()
  ...
  await models.page.getByRole("button", { name: "Diseñar tarjeta, marco y efectos", exact: true }).click()
  await rendered(models.page)
  await models.page.getByText("Compatibilidad con escenas anteriores",{exact:true}).click()
  await models.page.getByRole("button",{name:"Abrir modo anterior",exact:true}).click()
  ...
  await models.page.getByText("Plantillas anteriores · ornamentos 3D",{exact:true}).click()
  await models.page.getByRole("button",{name:"Atlas astral",exact:false}).first().click()
  await models.page.getByRole("button", { name: "Objetos y efectos", exact: true }).click()
  ...
  await modelMobile.page.getByRole("button", { name: "Diseñar tarjeta, marco y efectos", exact: true }).click()
